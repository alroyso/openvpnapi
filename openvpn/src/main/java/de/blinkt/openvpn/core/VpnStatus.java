...
*/
    ...
    private static ConnectionStatus getLevel(String state) {
        String[] noreplyet = {"CONNECTING", "WAIT", "RESOLVE", "TCP_CONNECT"};
        String[] nRECONNECTING = {"RECONNECTING"};
        String[] reply = {"AUTH", "GET_CONFIG", "ASSIGN_IP", "ADD_ROUTES"};
        String[] connected = {"CONNECTED"};
        String[] notconnected = {"DISCONNECTED", "EXITING"};

        for (String x : nRECONNECTING)
            if (state.equals(x))
                return ConnectionStatus.LEVEL_RECONNECTING;
